...
db.serialize(() =>  {
    ...
    //     db.run(insert, ["tomato", "sauce", "ts@example.com", "tomatocause", "qwrty12345", "Dallas"])
        }
    })
    db.run(
    `CREATE TABLE IF NOT EXISTS post (
        ...
        date_created TEXT,
        date_updated TEXT,
        user_id INTEGER NOT NULL, FOREIGN KEY (user_id) REFERENCES user (user_id) ON DELETE CASCADE
        )`,
        (err) => {
            if (err) {
                console.log("Table already created", err)
            ...
            //     db.run(insert, ["My third post", "there's gonna be a little tree. Let's make a nice big leafy tree. Paint anything you want on the canvas. Create your own world. Tree trunks grow however makes them happy.", "09/04/2023", "09/04/2023", "3"])
            }
    })
